...
async function getAllGradesForAdmin() {
  const [rows] = await pool.query(`
    ...
    FROM (
      SELECT id, user_id, student_id, mapel_id, semester, nilai, created_at, 'Tugas' AS grade_type FROM assignments
      UNION ALL
      SELECT id, user_id, student_id, mapel_id, semester, nilai, created_at, 'UTS' AS grade_type FROM midterms
      UNION ALL
      SELECT id, user_id, student_id, mapel_id, semester, nilai, created_at, 'UAS' AS grade_type FROM finals
    ) AS combined
...
async function getUserGradeRows(tableName, userId, gradeType) {
  const [rows] = await pool.query(
    `SELECT g.*, s.nama AS student_name, mp.nama_mapel AS subject_name, ? AS grade_type
     FROM ${tableName} g
...
app.get('/api/dashboard/stats', authMiddleware, async (req, res) => {
  try {
    ...
    const [gradesCount] = await pool.query(
      `SELECT ((SELECT COUNT(*) FROM assignments WHERE user_id = ?) + (SELECT COUNT(*) FROM midterms WHERE user_id = ?) + (SELECT COUNT(*) FROM finals WHERE user_id = ?)) AS count`,
      [req.user.id, req.user.id, req.user.id]
    );
    const [avgScoreRows] = await pool.query(
      `SELECT AVG(nilai) AS avg_score FROM (SELECT nilai FROM assignments WHERE user_id = ? UNION ALL SELECT nilai FROM midterms WHERE user_id = ? UNION ALL SELECT nilai FROM finals WHERE user_id = ?) AS all_scores`,
      [req.user.id, req.user.id, req.user.id]
    );
...
app.get('/api/admin/teachers', authMiddleware, adminOnly, async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT id, name, email, role, created_at FROM users WHERE role = ? ORDER BY name ASC, email ASC', ['teacher']);
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/admin/students', authMiddleware, adminOnly, async (req, res) => {
  try {
    const [rows] = await pool.query(
      `SELECT s.*, s.nama AS name, u.name AS teacher_name, u.email AS teacher_email FROM students s LEFT JOIN users u ON s.user_id = u.id ORDER BY s.created_at DESC`
    );
    res.json(rows);
  } catch (error) {
    res.status(500).json({ error: error.message });
